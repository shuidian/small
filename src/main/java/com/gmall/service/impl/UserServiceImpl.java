...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> resetPassword(String passwordOld,String passwordNew,User user){
        // 防止横向越权，要交验一下这个用户的旧密码，一定要指定是这个用户，因为我们会查询一个count(1)，如果不指定id，那么结果就是true了：count>0
    ...
    public ServerResponse<User> updateInformation(User user){
        ...
        if(updateCount > 0){
            return ServerResponse.createBySuccess("更新个新信息成功",updateUser);
        }
        return ServerResponse.createByErrorMessage("更新更人信息失败");
    }

    public ServerResponse<User> getInfirmation(Integer userId){
